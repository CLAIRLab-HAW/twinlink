...
``TwinTaskSim._hide_robot_collision_geoms`` must not decide by exclusion -- "a body whose name does not start with the
app's ``scene_prefix`` is robot". Under that rule an app that places a body under a name of its own (the sufficiency
study's ``task_object``) has it moved into the hidden geom group, which ``_ensure_renderer`` switches off for RGB *and*
depth: the object is invisible to every camera while every pose query keeps returning the truth. Nothing fails -- the
cameras simply see the surface behind it.

The rule is positive (``body_rootid`` == the robot's root), and this test holds it there.  Robot- and task-free MJCF, no
URDF bundle needed.
"""
