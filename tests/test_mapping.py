...
def test_from_yaml_loads_the_a200_mapping():
    ...
    for cand in ws.parents:
        if (cand / "workspace.repos").is_file():
            yaml_path = cand / "apps" / "spact-integration-demos" / "configs" / "a200_0553.yaml"
            break
    else:
...
def test_decode_joint_states_with_remap():
    ...
    s = RobotState()
    msg = NS(header=_header(), name=["ros_a", "b"], position=[0.5, 1.5], velocity=[0.1, 0.2], effort=[])
...
def test_decode_tf():
    ...
    tr = NS(
        ...
        transform=NS(translation=NS(x=1.0, y=2.0, z=3.0), rotation=NS(x=0.0, y=0.0, z=0.0, w=1.0)),
    )
...
def test_decode_odom_relative_to_start():
    ...
    def odom(x, y):
        return NS(
            header=_header(frame="odom"),
            child_frame_id="base_link",
            pose=NS(pose=NS(position=NS(x=x, y=y, z=0.0), orientation=NS(x=0.0, y=0.0, z=0.0, w=1.0))),
        )
...
def test_decode_camera_info():
    ...
    s.set_camera(
        "cam",
        CameraFrame(image=np.zeros((2, 2, 3), np.uint8), encoding="rgb8", stamp=0.0, frame_id="f", width=2, height=2),
    )
    m.apply("/r/info", "sensor_msgs/msg/CameraInfo", NS(header=_header(), k=K), s)
...
def _pointcloud(points_xyz, extra_nan=0):
    ...
    fields = [NS(name=n, offset=o, datatype=7) for n, o in (("x", 0), ("y", 4), ("z", 8))]  # 7 = FLOAT32
    return NS(
...
def test_decode_points_into_obstacles():
    ...
    s = RobotState()
    m.apply("/r/points", "sensor_msgs/msg/PointCloud2", _pointcloud([(0.0, 0.0, 1.0)]), s)
...
def _jpeg_msg(bgr):
    ...
    assert ok
    return NS(header=_header(frame="cam_frame"), format="rgb8; jpeg compressed bgr8", data=buf.reshape(-1))
...
def _depth_png_msg(depth_mm):
    ...
    return NS(
        header=_header(frame="depth_frame"),
...
def _lazy_mapping():
    return _mapping(
        cameras=[
            CameraMap(name="cam", image_topic="/r/img", info_topic="/r/info", lazy_decode=True),
            CameraMap(name="depth", image_topic="/r/dimg", is_depth=True, lazy_decode=True),
        ]
    )
...
def test_lazy_rvl_rejected_live():
    ...
    with pytest.raises(ValueError, match="RVL"):
        decode_compressed_bytes(payload, "16UC1; compressedDepth rvl", is_depth=True, allow_rvl=False)
    frame = CameraFrame(image=None, raw=payload, raw_format="16UC1; compressedDepth rvl", is_depth=True)
...
def test_camera_info_cached_but_reattaches_after_clear():
    ...
    def frame():
        return CameraFrame(
            image=np.zeros((2, 2, 3), np.uint8), encoding="rgb8", stamp=0.0, frame_id="f", width=2, height=2
        )
    ...
    m.apply(
        "/r/info", "sensor_msgs/msg/CameraInfo", NS(header=_header(), k=[0.0] * 9), s
    )  # garbage ignored: cached K wins
    assert s.camera("cam").intrinsics[0, 0] == 600.0


def test_recv_stamp_lands_on_frame():
    m, s = _lazy_mapping(), RobotState()
    m.apply("/r/img", _COMPRESSED_TYPE, _jpeg_msg(np.zeros((2, 2, 3), np.uint8)), s, recv_stamp=123.456)
