...
def _pointcloud(points_xyz, extra_nan=0):
    ...
    fields = [
        NS(name=n, offset=o, datatype=7)  # 7 = FLOAT32
...
def test_decode_points_into_obstacles():
    ...
    s = RobotState()
    m.apply(
        "/r/points", "sensor_msgs/msg/PointCloud2", _pointcloud([(0.0, 0.0, 1.0)]), s
    )
...
def _jpeg_msg(bgr):
    ...
    assert ok
    return NS(
        header=_header(frame="cam_frame"),
...
def _lazy_mapping():
    return _mapping(
        cameras=[
            CameraMap(
                name="cam", image_topic="/r/img", info_topic="/r/info", lazy_decode=True
            ),
            CameraMap(
                name="depth", image_topic="/r/dimg", is_depth=True, lazy_decode=True
            ),
        ]
    )
...
def test_lazy_rvl_rejected_live():
    ...
    payload = (
        struct.pack("<Iff", 0, 100.0, 0.0) + struct.pack("<II", 2, 2) + b"\x00" * 8
    )
    with pytest.raises(ValueError, match="RVL"):
        decode_compressed_bytes(
            payload, "16UC1; compressedDepth rvl", is_depth=True, allow_rvl=False
        )
    frame = CameraFrame(
        image=None, raw=payload, raw_format="16UC1; compressedDepth rvl", is_depth=True
    )
...
def test_recv_stamp_lands_on_frame():
    ...
    m.apply(
        "/r/img",
        _COMPRESSED_TYPE,
        _jpeg_msg(np.zeros((2, 2, 3), np.uint8)),
        s,
        recv_stamp=123.456,
    )
