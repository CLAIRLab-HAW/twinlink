...
class MujocoSink(StateSink):
    ...
    def _load_model(self, mujoco):
        ...
        if path.endswith(".urdf"):
            ...
            return load_mujoco_from_urdf(
                path, floating_base=self.base_free_joint is not None, keep_visual=self.keep_visual, colored=self.colored
            )
    ...
    def _camera_link_candidates(self) -> tuple[str, ...]:
        ...
        return tuple(
            name for prefix in self.spec.hand_prefixes for name in (f"{prefix}_link", f"{prefix}_bottom_screw_frame")
        )
        ...
        for world in self._world_clouds(mujoco):
            for p in self._voxelize(world):
                if scene.ngeom >= scene.maxgeom:
                    break
                mujoco.mjv_initGeom(
                    scene.geoms[scene.ngeom], mujoco.mjtGeom.mjGEOM_BOX, size, np.ascontiguousarray(p, float), eye, rgba
                )
                scene.ngeom += 1
    ...
    def _render(self, mujoco) -> bool:
        ...
        if self._renderer is not None:
            ...
            if ghost_positions is not None and frame is not None:
                log.debug("ghost_positions: %d joints, state=%s", len(ghost_positions), self._preview_state)
    ...
    def _alpha_blend(base: np.ndarray, overlay: np.ndarray, alpha: float = 0.45) -> np.ndarray:
        ...
        return ((1.0 - alpha) * base.astype(np.float32) + alpha * overlay.astype(np.float32)).astype(np.uint8)
