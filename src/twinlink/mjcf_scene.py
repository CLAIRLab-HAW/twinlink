...
OBSTACLE_BODY_PREFIX = "hrl_obstacle_"
DISTRACTOR_BODY_PREFIX = "hrl_distractor_"
...
def obstacle_body_name(index: int) -> str:
    return f"{OBSTACLE_BODY_PREFIX}{index}"
...
def add_obstacle_pool(worldbody: ET.Element, n_slots: int) -> None:
    ...
    for i in range(int(n_slots)):
        ...
        body.set("name", obstacle_body_name(i))
        body.set("pos", fmt(px + 0.5 * i, py, pz))
        geom = ET.SubElement(body, "geom")
        geom.set("name", f"{obstacle_body_name(i)}_geom")
        geom.set("type", "box")
        geom.set("size", fmt(0.02, 0.02, 0.02))
        geom.set("rgba", "0.85 0.45 0.10 0.55")


def distractor_body_name(index: int) -> str:
    return f"{DISTRACTOR_BODY_PREFIX}{index}"


def distractor_joint_name(index: int) -> str:
    return f"{DISTRACTOR_BODY_PREFIX}{index}_free"


def add_distractors(worldbody: ET.Element, distractors: Sequence[Dict]) -> None:
    ...
    for i, spec in enumerate(distractors):
        ...
        body = ET.SubElement(worldbody, "body")
        body.set("name", distractor_body_name(i))
        body.set("pos", fmt(*pos))
        if abs(yaw) > 1e-9:
            body.set("quat", fmt(np.cos(yaw / 2.0), 0.0, 0.0, np.sin(yaw / 2.0)))
        if spec.get("dynamic"):
            free = ET.SubElement(body, "freejoint")
            free.set("name", distractor_joint_name(i))
        geom = ET.SubElement(body, "geom")
        geom.set("name", f"{distractor_body_name(i)}_geom")
        geom.set("type", "box")
